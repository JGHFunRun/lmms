...
void SampleClipView::updateSample()
{
	...
	ToolTip::add( this, ( m_clip->m_sample.sampleBuffer()->filePath() != "" ) ?
					PathUtil::toAbsolute(m_clip->m_sample.sampleBuffer()->filePath()) :
...
void SampleClipView::dropEvent( QDropEvent * _de )
{
	...
	{
		m_clip->m_sample.setSampleData(StringPairDrag::decodeValue( _de ), SampleBufferV2::StrDataType::Base64);
		m_clip->updateLength();
		update();
		_de->accept();
...
{
	QString af = m_clip->m_sample.openSample();

	if ( af.isEmpty() ) {} //Don't do anything if no file is loaded
	else if ( af == m_clip->m_sample.sampleBuffer()->filePath() )
	{	//Instead of reloading the existing file, just reset the size
		int length = (int) ( m_clip->m_sample.sampleBuffer()->numFrames() / Engine::framesPerTick() );
...
void SampleClipView::paintEvent( QPaintEvent * pe )
{
	...
	m_clip->m_sample.visualize( p, r, pe->rect() );

	QString name = PathUtil::cleanName(m_clip->m_sample.sampleBuffer()->filePath());
